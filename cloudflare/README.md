...
- `GET /documents/:id/file` — streams the original file back with its real content-type and `Content-Disposition: inline`, so the dashboard's document name links open a browser-native preview (PDF viewer, plain text) in a new tab instead of forcing a download.

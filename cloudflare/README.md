...
- `POST /resumes/master` — builds the master archive: every role, every accomplishment, no selection pressure and no page budget. See "The résumé doctrine agent" below. One per profile, replaced in place on regeneration.
...
- `POST /companies/discover` (`{"provider"?, "count"?: 1-20, "focus"?: string}`) — proposes companies from the structured profile and Desired Roles, then **verifies each proposed site actually resolves** before trusting it. **A proposal that doesn't resolve is never added at all** — not added and flagged — because a model listing employers will occasionally invent or misremember one (Abundant Robotics, a real company that shut down and let its domain lapse, is exactly this case), and a dead entry the candidate can't act on is worse than not showing it. `verifyWebsite()` still tells a genuinely dead domain (no response at all — DNS failure, connection refused, timeout) apart from a live site that just blocks scrapers (401/403, treated as reachable): punishing every company with a bot-blocking WAF in front of an otherwise normal site would throw away real, live employers over nothing. Companies already on the list are passed into the prompt as exclusions *and* deduped on insert by a normalized name key, so `Acme, Inc.` and `Acme Inc` can't both land.
- `POST /companies/scan` (`{"limit"?: 1-12, "company_id"?: string}`) — reads job boards for companies that need it and writes the results into `job_postings`. Streams progress as newline-delimited JSON (see "Streaming progress" below) ending in `{"type":"done","scanned","results","new_listings","unscanned"}`, where `new_listings` is rows actually inserted this call via `meta.changes` — not just how many matched, since a re-scan legitimately re-matches postings it already has. Driven from the Search tab's "Scan for new listings" step for all companies at once; the per-company "Scan" button on the Companies tab calls the same endpoint with `company_id` set.

**No aggregators, by design.** Jobs come from each company's own board. Nearly every company runs that board on one of a handful of applicant tracking systems, and five of them publish plain public JSON APIs this app reads directly: Greenhouse (`boards-api.greenhouse.io`), Lever (`api.lever.co`), Ashby (`api.ashbyhq.com`), SmartRecruiters (`api.smartrecruiters.com`), and Workday (`*.myworkdayjobs.com`'s CXS API). So "check their site directly" is implemented by resolving *which* ATS a company uses and reading that board's API, which is structured and stable, rather than scraping a JavaScript-rendered careers page.

**A dozen more platforms are recognized without being auto-read.** ADP, iCIMS, BambooHR, Workable, Recruitee, JazzHR, Breezy, Personio, Paylocity, UKG/UltiPro, SAP SuccessFactors, Oracle Taleo, and Jobvite are all detected by their careers-link URL pattern (`ATS_PATTERNS`/`isReadableAtsProvider` in `src/companies.ts`), but none of them publish a documented public API the way the five readable providers do — several (ADP chief among them) are fully server-rendered with nothing to point a JSON parser at. Recognizing the platform is still real progress over silence: a company using one of these gets `ats_provider` set to the platform's name, `careers_url` updated to the *actual* board link found (rendered as a "View job board ↗" link on the Companies tab), and a scan note reading "Uses ADP for hiring. View current openings directly." instead of the old, misleading "No supported job board found on their site." — which read as "nothing exists here" when something very much did, the candidate just had to go find it by hand. Guessing a JSON shape for these with no way to verify it against a live response is how you ship code that silently returns nothing while looking like it works, so the honest move is detection plus a working link, not a parser nobody can check.
...
4. **Last resort: guess plausible org slugs** (from the domain and the company's name) against each of the five readable providers' APIs directly — for a careers page that renders its board client-side and never puts the real link in the static HTML at all.
...
**`status = 'unreachable'` is now a purely historical value.** Before this pass, a proposed company whose site didn't resolve was still added, just flagged; `POST /companies/discover` now skips it entirely instead (see above), so nothing in the app writes this status any more. It's kept in the schema and in the bulk-scan exclusion query only for rows a database already had before this change, or a status set some other way — a fresh discovery run will never create one again. A company found to use a detected-but-unreadable ATS platform (`ats_provider` set to `adp`, `icims`, and the rest — see above) gets the same "can't be scanned" treatment on the dashboard as genuine `ats_provider = 'none'` misses, since no postings will ever appear for either automatically — the difference is that the detected-provider case comes with a working link to check by hand, and the truly-nothing-found case doesn't. Unlike `unreachable`, it isn't filtered out of the bulk-scan target query by `ats_provider` — it stays eligible for the daily re-scan window like any other company — but `scanOneCompany`'s readability check runs before `resolveBoard` or any board fetch, so a re-included company like this costs one cheap status-preserving write, not a repeat of the actual (already-failed-to-be-worth-retrying) resolution work. The per-company `POST /companies/scan {"company_id"}` call has no status filter at all, so the Companies tab's per-row "Scan" button always works as a deliberate manual retry (the site may have come back up, or the company may be worth checking again after this pass's changes).
...
**A company scanned earlier the same day is skipped on the next bulk scan, unless `force` says otherwise.** `POST /companies/scan`'s target query excludes anything with `last_scanned_at` from today (`date(last_scanned_at) < date('now')`, in addition to never-scanned companies), and the `unscanned` count in its response is filtered the same way — so re-running a scan later the same day doesn't re-pay to re-read a board it just read, and the status line ("N companies still to scan today") reflects only what's actually left before the calendar day rolls over. A single-company rescan (by id, e.g. a manual retry) is unaffected — that path has always been a deliberate, explicit action.

This guard actively works against exactly one legitimate case: after a change to what gets *captured* from a posting (the description-cap widening above is the case that surfaced this), every already-scanned posting is stuck on its old, thinner text until it's re-read — and the ordinary daily guard silently no-ops that very re-scan, reporting "0 scanned" with no indication anything was skipped rather than an error to investigate. `POST /companies/scan {"force"?: boolean}` lifts the same-day exclusion when true (still respects `dismissed`/`unreachable` status). The Search tab's "Re-scan companies already read today too" checkbox drives it, off by default so the common case (don't waste budget re-reading a board just read) is unaffected; when a bulk scan finds nothing to do because everything was already scanned today, the status line says so explicitly and points at the checkbox, instead of reading identically to "ran fine, nothing new" — that ambiguity was the actual reason a widened description cap didn't visibly do anything on a first re-scan attempt.

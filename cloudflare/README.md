...
**The workflow for an Interested job is Strengthen Profile → Resume → Cover Letter → Apply**, in that order, and the ordering is the argument: the résumé should be written from the best version of the candidate's evidence, so the step that improves the evidence comes first.

Opening it analyzes the posting (lazily — nothing runs until you ask, which is what keeps twenty interesting jobs from costing twenty rounds of model calls), grades every requirement against the career record as proven/partial/unproven, and asks a short list of targeted questions where an answer could plausibly uncover real evidence the record doesn't yet hold. One long scrollable page, one **Update Profile** button at the bottom.

The answers go into the **canonical profile**, not into a per-job scratchpad. That is the whole point: evidence remembered while looking at job A is already there when job B is analyzed, so the number of questions falls as the profile fills in. Question history is stored profile-wide (`job_id` is provenance, not ownership), which is what makes "don't ask this again" work across postings — including a dismissal, since "I haven't done this" is a real answer worth remembering.

This **replaced two things**: the Profile tab's generic **Improve** subtab, and this section's older freeform "ask one question" assistant. A profile-wide audit has no basis for ranking one gap above another, so it asked broad questions; a posting supplies that ranking for free. The machinery underneath was reused rather than rebuilt — the question table, the apply/merge pass, and the `/profile/improve/*` endpoints are what Strengthen Profile runs on, and `extractJobRequirements`/`planEvidence` were already there inside the résumé builder, just being thrown away after every build.

Full design, stages, schema and truthfulness rules: [`docs/workflow/strengthen-profile.md`](../docs/workflow/strengthen-profile.md). House résumé style, and how the r/EngineeringResumes wiki was folded into it: [`docs/product/resume-guidelines.md`](../docs/product/resume-guidelines.md).

### Job assistant (superseded)

> The freeform per-job question flow described below has been **replaced in the UI by Strengthen Profile**. The `/jobs/:id/review` and `/jobs/:id/review-answer` endpoints are still served and still read — the browser extension and the application-answer generator use that evidence, and existing answers still reach résumé and cover-letter generation — but nothing in the dashboard writes them by hand any more. Kept here because the evidence-handling rules below still govern how those stored answers are used.
...
Composing draws on whatever's already available for this job: the profile, the job description, any answered `job_review` evidence, and — if a job-tailored résumé already exists — its `contact_line`, so the letterhead doesn't have to guess at contact details or invent one.

**It now also receives the Strengthen Profile analysis**, via the `job_analysis` variable: the role summary, the strongest *verified* connections ranked by how important the employer says they are, partial matches flagged as do-not-overstate, and unproven requirements as an explicit prohibition. Without it, a letter can cheerfully claim a requirement the candidate can't prove — the exact failure the analysis exists to prevent — and enthusiasm phrased as experience ("excited to bring my Kubernetes experience" when there is none) is the most common form of it. The rendering is deliberately shaped differently from the résumé's `renderPlanDirective`: a résumé needs a per-role space budget and the full requirement checklist, a letter needs the opposite — two or three connections developed properly, and a hard stop on everything else. Handing the letter writer the résumé's directive is what produces a letter that just paraphrases the résumé. Jobs never analyzed pass an empty string and behave exactly as before; the analysis grounds the letter, it doesn't gate it. **One cover letter exists per job**, same storage pattern as the tailored résumé: `cover_letters.job_id` is unique, the existing letter is returned as-is unless the user clicks "Regenerate for this job" (which updates the row in place, no revision history since a cover letter is cheap to redo and always singular per application).
...
**Coverage before optimization.** `extractJobRequirements()` reads a posting into discrete, classified requirements (`must_have` / `responsibility` / `preferred` / `competency`) rather than treating it as a bag of words; `planEvidence()` then reports each as proven, partial, or unproven. Checking each stated requirement individually is the point — a globally "similar-looking" résumé can quietly drop the one line proving a required certification while keeping five attractive but less consequential ones. Requirements are cached on `job_postings.requirements_json` (they don't change between revisions). The graded plan is a property of the posting *crossed with the current profile*, so it changes whenever the profile gains evidence — an answer given on Strengthen Profile can legitimately turn an unproven requirement into a proven one. Both are now persisted together per job in `job_evidence_analysis` (migration `0033`) by the Strengthen Profile workflow, and the résumé and cover-letter writers read that saved artifact instead of re-deriving it: two fewer model calls per build, and the documents agree with the coverage the candidate was just shown rather than forming a second opinion. See [`docs/workflow/strengthen-profile.md`](../docs/workflow/strengthen-profile.md).

**The anti-fabrication backstop is in code, not the prompt, and it is two checks.** First, `normalizePlan()` demotes any `proven` claim whose cited evidence doesn't actually trace back to the profile. This is deliberately a content-word overlap test rather than exact substring matching: the model is *asked* to paraphrase ("delivered CRM onboarding" for "taught staff to use our customer database"), and an exact-match check would reject the honest case constantly. What distinguishes fabrication is that an invented accomplishment shares almost no specific vocabulary with the profile at all. Second — and this is the harder failure — a claim also has to show that the *requirement's own distinctive vocabulary* appears somewhere in the record. The first check only asks "is this evidence real?", which a model satisfies while citing the wrong thing: quoting genuine Plotly work as proof of a `D3.js` requirement passes it, because every word of the quote really is in the profile. When the requirement names something the record never mentions, the grade becomes `partial` rather than `unproven` — there is real adjacent experience, it simply isn't the named thing, and saying so is more useful than either silence or a lie. The rule that falls out: **adjacent experience is never upgraded into exact experience**; ApplyGo may notice related work exists and *ask* whether the exact thing also happened, but it may not answer that question on the candidate's behalf. The same function also defaults a role the model skipped to `compress` rather than letting it silently vanish, and derives every bullet budget from the level. Unproven requirements reach the writer as an explicit prohibition ("write nothing that claims, implies, or hints at these"), because left implicit, a writer that knows the target wants Kubernetes will reliably find some way to gesture at Kubernetes.
...
Prompts carrying a bundled default are whatever `PROMPT_DEFAULTS` in [`src/prompts.ts`](src/prompts.ts) lists — currently `profile/create`, `profile/improve-audit`, `profile/improve-apply`, `job/strengthen-questions`, `cover_letter/compose`, `roles/analyze`, and `roles/research`. Of those, `profile/create`, `profile/improve-audit`, `profile/improve-apply`, `roles/analyze`, and `roles/research` already have a matching `production` version in Langfuse, so the bundled text is dormant for them; `job/strengthen-questions` and `cover_letter/compose` are new (the latter needs a new `job_analysis` version of an existing prompt) and still need their first sync. Rather than copying text by hand, run the sync script, which reads the registry directly and applies the same staleness rule the runtime does:

```bash
npm run prompts:check   # dry run -- what is missing, stale, or edited in Langfuse. Writes nothing.
npm run prompts:sync    # create/update the ones that need it, labeled production
```

Needs `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, and (outside the default region) `LANGFUSE_BASE_URL`, from the environment or the gitignored `.env`/`.dev.vars`. A prompt someone has deliberately edited in Langfuse is reported but **not** overwritten unless you pass `--all`. Full details, including the variables each prompt expects, are in [`docs/technology/langfuse-prompt-sync.md`](../docs/technology/langfuse-prompt-sync.md).

One rule when adding a prompt to the registry: `requires` must name only variables that are **new** to the change being shipped. The check is `.some()`, so listing a variable the previous template already had would make the live version pass the staleness test forever and the bundled text would never be served.

...
Prompts carrying a bundled default are whatever `PROMPT_DEFAULTS` in [`src/prompts.ts`](src/prompts.ts) lists — currently `profile/create`, `profile/improve-audit`, `profile/improve-apply`, `job/strengthen-questions`, `cover_letter/compose`, `roles/analyze`, and `roles/research`. Rather than copying text by hand, run the sync script, which reads that registry directly and applies the same staleness rule the runtime does:

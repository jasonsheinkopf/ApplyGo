...
Storage is bounded on the way in: scraped descriptions are capped at 4000 characters (matching the scrape cap in `fetchBoardJobs`, `src/companies.ts` -- storage was the actual bottleneck below that, not the scrape). Descriptions are the largest column in the database, and the Data tab reports exactly how much space they take.

**A too-tight description cap silently starved the "what do you care about" facts.** The cap here used to be 1500 characters, sized for tier 1's title-only screen and tier 2's requirements-matching -- both of which read the *opening* of a posting. But compensation, remote/onsite policy, hours, and travel routinely sit in their own section at the very *end* of a real posting (a "Compensation and Benefits" block after every requirements bullet), well past where 1500 characters cuts off -- so those facts were never in what the model saw, and it correctly (per its own instructions) reported "Not specified" for something it was simply never given. Raising the cap to 4000 on the way in, and matching it in `fitPrompt`'s own per-job slice (`src/fit.ts`, was also 1500), fixes this going forward. A posting already on file from before this fix stays stuck at its old truncated text forever otherwise -- `POST /companies/scan`'s insert loop now backfills it: when a posting's `(company_id, external_id)` already exists (an `INSERT OR IGNORE` no-op), a follow-up `UPDATE ... WHERE LENGTH(raw_description) < LENGTH(?)` replaces the stored description only if the freshly scraped one is actually longer, so a plain re-scan is enough to pick up the fuller text -- no need to delete and re-add anything. Re-running "Re-check my top matches" afterward re-scores against the now-complete description.

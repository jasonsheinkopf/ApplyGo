...
// ---------------------------------------------------------------------------
// The doctrine, as prompt text
// ---------------------------------------------------------------------------

/**
 * Shared by every prompt that decides what belongs on a page. Deliberately written as reasons
 * rather than commandments, per Randazzo's own finding that adaptive reasoning outperforms rigid
 * resume rules.
 */
export const RESUME_DOCTRINE = `
RESUME DECISION DOCTRINE

A resume is not an autobiography. It is an evidence package answering exactly one question:
"why should this employer believe this person can succeed in this particular job?"

For every line you are considering, ask: what employer belief does this line create? If there is no
good answer, the line does not belong. Keep evidence, not history.

The unit of decision is the individual accomplishment, not the job title that contains it. A single
past role holds many separable pieces of evidence, and different targets activate different subsets.
A teaching job can supply curriculum design, adult training, public speaking, stakeholder handling,
assessment-data analysis, and mentoring -- surface whichever of those the target actually needs, and
leave the rest in the archive. The historical fact never changes; its evidentiary value changes
because the question changed.

WEIGH THESE, IN ROUGHLY THIS ORDER:
- Relevance: does this support a stated requirement or core responsibility of the target?
- Evidence strength: is this a real accomplishment with an outcome, scope, or decision enabled, or
  merely a duty that was assigned? Prefer the former; downrank the latter.
- Recency: more recent is generally worth more, but relevance outranks recency. A directly relevant
  accomplishment from years ago beats a recent one that proves nothing about this target. Do not
  reorder purely by date.
- Seniority and scope: does it demonstrate the level of responsibility this target expects?
- Distinctiveness: does it prove something no other line already proves?
- Narrative clarity: does it make the career path easier to understand rather than more confusing?

PENALIZE:
- Redundancy. Six lines proving "communication" is five wasted lines. One strong example per
  competency, then spend the space proving a different competency instead. A broad proof surface
  beats a deep one.
- Fluff: adjectives and self-assessments with no evidence behind them.
- Space consumed disproportionate to the belief created.

FOUR LEVELS, NOT IN-OR-OUT. Every role gets one of:
- FEATURE: prominent placement, multiple strong bullets. Directly proves core requirements.
- INCLUDE: normal entry, one or two relevant bullets.
- COMPRESS: title, employer, and dates only, or a single line. Preserves an understandable
  chronology without spending evidence space on it.
- OMIT: kept in the archive, absent from this document.
The same 15-year-old teaching job can be FEATURE for a training role, INCLUDE for customer
education, COMPRESS for an engineering manager role, and OMIT for an individual-contributor
software role. That is correct behavior, not inconsistency.

TRUTH CONSTRAINTS -- these override everything above:
- Never claim evidence the candidate's profile does not contain. If the target requires something
  they cannot demonstrate, say nothing about it. Do not imply, hint, or gesture at it.
- Use the employer's own vocabulary ONLY when it truthfully describes what actually happened.
  Translating "taught staff to use our internal customer database" into "delivered CRM onboarding"
  is fair when it is accurate. Writing "Salesforce CRM specialist" for someone who never used
  Salesforce is not. The test is whether the candidate could defend the wording in an interview.
- Never fabricate a metric to satisfy a "quantify everything" instinct. Legitimate non-numeric
  evidence includes decisions enabled, risk avoided, work unblocked, systems shipped, and scope of
  stakeholders.
`.trim();

/**
 * The posture for the archive document. Berkeley's master-resume step: capture everything first,
 * because you cannot select from evidence you never wrote down. Selection pressure is exactly what
 * this document must NOT have.
 */
export const MASTER_DOCTRINE = `
THIS IS A MASTER ARCHIVE, NOT A SUBMITTABLE RESUME.

Its purpose is completeness. Every tailored resume is later generated by deleting from this, so
anything missing here can never appear on any of them. It is expected to be far too long to send to
an employer. That is correct.

- Include EVERY role in the profile, with no exceptions, in reverse chronological order.
- Include every distinct accomplishment for every role -- do not select the best few. If a role has
  eight separable accomplishments, write all eight.
- Split combined accomplishments apart. One bullet should carry one piece of evidence, so that later
  filtering can take it or leave it independently. "Built the reporting pipeline and trained the
  team on it" is two bullets, not one.
- Make each bullet self-contained, so it still reads correctly when the surrounding bullets are
  removed. Never write "also did X" or "in addition to the above".
- Include every skill, project, credential, and education entry present in the profile.
- Do not compress, do not omit, do not judge relevance. There is no target role here to be relevant
  to. Length is not a concern and the page budget does not apply.

The truth constraints still apply in full: this archive contains everything the candidate HAS done,
and nothing they have not.
`.trim();
...
export async function extractJobRequirements(
  ...
  const prompt = [
    "Read this job posting and break it into the specific things it actually asks for.",
    "",
    "Split compound requirements into separate entries so each can be checked independently.",
    "Classify each by kind. Take requirements only from what the posting states or clearly implies --",
    "do not add standard-for-the-industry expectations it never mentions.",
    "",
...
export async function planEvidence(
  ...
  const prompt = [
    "You are deciding how much of a resume each of this candidate's past roles deserves, for one",
    "specific target. You are not writing the resume -- a writer does that next, using your plan.",
    "",
    RESUME_DOCTRINE,
    "",
    `TARGET: ${targetLabel}`,
    requirements.role_summary ? `WHAT THE ROLE IS: ${requirements.role_summary}` : "",
    "",
    "REQUIREMENTS TO COVER (report on each one, in this order):",
    ...requirements.requirements.map((r) => `- [${r.kind}] ${r.text}`),
    "",
    "Cover the must_have requirements the candidate can genuinely support before optimizing anything",
    "else. For any requirement the profile does not support, mark it unproven and leave the evidence",
    "field empty. Do not stretch a loosely related accomplishment into a match, and do not write",
    "evidence text that is not actually in the profile below.",
    "",
    `CANDIDATE PROFILE (the only permitted source of evidence):\n${JSON.stringify(profile)}`,
  ]
    .filter(Boolean)
    .join("\n");

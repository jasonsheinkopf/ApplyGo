...
<style>
  ...
  * { box-sizing: border-box; }
  /* Every page is one shell with a single .panel toggled visible at a time (see .panel/.panel.active
     below), so there is only ever one container width -- but a taller panel's own vertical
     scrollbar (present) vs. a shorter panel's lack of one (absent) still changes the *available*
     horizontal width on desktop browsers with non-overlay scrollbars, which is what actually moved
     the centred .shell sideways by half a scrollbar's width when switching tabs. Reserving the
     scrollbar's space unconditionally removes that variable rather than patching each page's width.
     overflow-y: scroll is the reliable cross-browser mechanism; scrollbar-gutter is additive where
     supported. overflow-x: hidden is a plain safety net against the same kind of shift from any one
     panel's content accidentally overflowing horizontally. */
  ...
  /* Main nav: a chevron/process-step sequence (You > Career > CV > Companies > Jobs > Settings)
     rather than a row of unrelated pills, so the shape itself communicates that the product flow
     moves left to right. Each button is a right-pointing arrow (clip-path) with a matching notch
     cut into its own left edge (all but the first button, which has a flat left edge); a small
     the gap between buttons lets the page background show through both the point and the notch as a
     thin chevron-shaped seam, which is what makes adjacent segments read as interlocking without
     relying on paint order between elements (a border can't follow a clip-path's diagonal edges,
     so segments are distinguished by background/gap alone, not borders).
     --nav-arrow is redeclared smaller under the narrow-viewport media query below so the same
     shape scales down instead of the row needing to scroll. */
  nav {
    --nav-arrow: 14px;
    display: flex; width: 100%; gap: 3px; overflow-x: auto; padding: 0; margin-bottom: 1.1rem;
    background: transparent; border: none;
    -webkit-overflow-scrolling: touch; scrollbar-width: none;
  }
  nav::-webkit-scrollbar { display: none; }
  /* Fallback only: the flexible equal-width buttons below are sized to fit the row without
     scrolling in the normal case, but an extremely narrow viewport (or a very long label) can
     still force overflow, so the same scroll + edge-fade affordance from before stays as a safety
     net rather than letting content clip invisibly. */
  nav.can-scroll-left {
  ...
  nav button {
    flex: 1 1 auto; min-width: 0; margin: 0; margin-top: 0;
    padding: 0.6rem calc(0.55rem + var(--nav-arrow) * 0.7) 0.6rem calc(0.55rem + var(--nav-arrow) * 0.5);
    font-size: 0.85rem; font-weight: 650; letter-spacing: -0.005em;
    background: var(--surface); border: none; color: var(--text-muted); cursor: pointer;
    text-align: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    transition: background 0.12s ease, color 0.12s ease;
    clip-path: polygon(
      0 0, calc(100% - var(--nav-arrow)) 0, 100% 50%, calc(100% - var(--nav-arrow)) 100%, 0 100%,
      var(--nav-arrow) 50%
    );
  }
  /* No incoming point to notch around, and rounded so the sequence has a clean cap rather than a
     sharp corner at the very start. */
  nav button:first-child {
    padding-left: 0.85rem;
    border-radius: var(--radius-sm) 0 0 var(--radius-sm);
    clip-path: polygon(0 0, calc(100% - var(--nav-arrow)) 0, 100% 50%, calc(100% - var(--nav-arrow)) 100%, 0 100%);
  }
  ...
  nav button:focus-visible { outline: 2px solid var(--accent); outline-offset: -2px; z-index: 1; position: relative; }
  /* A normal iPhone (390-430px) fits all six segments without help most of the time, but the
     tighter end of that range (and the "slightly smaller mobile width" bucket below it) needs a
     smaller arrow, tighter padding and font to still avoid the scrolling fallback -- see the
     comment on nav above for why scrolling is a fallback, not the default. */
  @media (max-width: 430px) {
    nav { --nav-arrow: 9px; gap: 2px; }
    nav button {
      padding: 0.55rem calc(0.4rem + var(--nav-arrow) * 0.7) 0.55rem calc(0.4rem + var(--nav-arrow) * 0.5);
      font-size: 0.76rem;
    }
    nav button:first-child { padding-left: 0.6rem; }
  }
  /* Below a normal iPhone's width, the shortest labels ("You", "CV") were shrinking harder than
     they needed to just because the longest ("Companies") was competing for the same space --
     a floor stops a short label being squeezed to a sliver, without being applied at the normal
     iPhone width above where it would instead crowd space away from the longer labels and make
     *those* truncate worse. Below the floor's own width the row falls back to the horizontal-
     scroll affordance (nav.can-scroll-*) instead of shrinking text further, per the "readable
     labels" priority over "always zero scroll". */
  @media (max-width: 380px) {
    nav button { font-size: 0.72rem; min-width: 2.6rem; }
  }
  .panel { display: none; }
  ...
  button {
    ...
    /* Skips the browser's ~300ms wait to see if a tap is the start of a double-tap-to-zoom gesture
       before it commits to being a click -- with this, every button (nav and subtab included)
       responds the instant it's tapped instead of feeling a beat slow on a touch screen. */
  ...
  /* Mobile replacement for a .segmented-control: pagination dots (current position) plus two
     ...
     so these buttons are the only way to move between subtabs on a phone, not just a backup for a
     gesture. Hidden by default -- the media query below is what turns it on and hides the
     segmented control it stands in for, only on screens too narrow to fit every sub-tab label on
     one line. */
  .mobile-subnav { display: none; }
  @media (max-width: 640px) {
    /* Visually hidden rather than display:none -- the buttons still do the real navigating (the
       dots/arrows are cosmetic, see initMobileSubnav below), so a screen reader or keyboard user
       needs them to stay reachable even though sighted mobile users no longer see them. */
    .segmented-control.has-mobile-subnav {
      position: absolute; width: 1px; height: 1px; padding: 0; margin: -1px; overflow: hidden;
      clip: rect(0, 0, 0, 0); white-space: nowrap; border: 0;
    }
    .mobile-subnav {
      display: flex; flex-direction: column; align-items: center; gap: 0.35rem; margin-bottom: 1rem;
    }
    .mobile-subnav-row { display: flex; align-items: stretch; justify-content: center; gap: 0.6rem; width: 100%; }
    .mobile-subnav-dots { display: flex; align-items: center; justify-content: center; gap: 0.4rem; flex: 0 0 auto; }
    .mobile-subnav-dot {
    ...
    /* The real way to move between subtabs on a phone (see the block comment above), so each
       arrow claims the empty horizontal space on its side of the dots instead of staying a small
       icon-only circle -- a wide, comfortably tall rounded-rectangle tap target, not a sliver. A
       max-width keeps them from growing absurdly wide on a tablet-sized "mobile" viewport. */
    .mobile-subnav-arrow {
      display: flex; align-items: center; justify-content: center; flex: 1 1 0;
      min-width: 3.25rem; max-width: 7rem; min-height: 3rem; border-radius: var(--radius-sm);
      border: 1px solid var(--border); background: var(--surface); color: var(--text); padding: 0;
    }
    .mobile-subnav-arrow svg { display: block; width: 20px; height: 20px; }
...
<body>
  ...
  <nav id="workflow-nav">
    <button class="tab active" data-tab="profile" type="button">You</button>
    <button class="tab" data-tab="careers" type="button">Career</button>
  ...
  <script>
    document.querySelectorAll('nav .tab').forEach(function (tabButton) {
      tabButton.addEventListener('click', function () {
        ...
        // there is more to scroll to. Instant, not smooth: the equal-width chevron nav now fits
        // without scrolling in the normal case (this only fires the rare fallback), and an
        // animated scroll on every tap was the actual source of the tab switch feeling laggy.
        tabButton.scrollIntoView({ behavior: 'auto', block: 'nearest', inline: 'center' });
      });
    });
    ...
    // two large previous/next tap targets, replacing the segmented control only on screens too
    // narrow to show every label at once (see the .has-mobile-subnav media query above; desktop
    // keeps the segmented control as-is). Explicit next/prev buttons are the only way to change
    // subtabs here -- horizontal swiping the page content deliberately does not, since a swipe
    // meant to scroll a long page (or just reposition a thumb) was silently changing the visible
    // subsection instead, which is confusing on a screen with real vertical content.
    //
    ...
    // a subpanel, refetch a list, whatever), so the arrows just click the next/previous button and
    // let that handler do its normal job. That also means the dots/label stay correct even when
    // something other than an arrow changes the active button (a plain click, or code elsewhere
    // calling .click()).
    ...
    [
      document.querySelector('#panel-profile .segmented-control'),
      document.querySelector('#panel-careers .segmented-control'),
      document.getElementById('companies-view-tabs'),
      document.getElementById('jobs-view-tabs'),
      document.querySelector('#panel-settings .segmented-control'),
    ].forEach(initMobileSubnav);
